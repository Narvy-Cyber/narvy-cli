"""iOS source (Swift/ObjC) SAST. Entry point: analyze_source()."""
...
def _run_swift_semgrep(source_dir: str,
    ...
    cmd = (
        [semgrep_engine.semgrep_bin() or "semgrep", "--config", SWIFT_SEMGREP_RULES_PATH, source_dir,
         "--include", "*.swift",
         "--json", "--quiet", "--no-git-ignore",
         "--timeout", "60"]
        + semgrep_engine.resource_args(semgrep_engine.semgrep_jobs())
        + exclude_args
    )
    try:
