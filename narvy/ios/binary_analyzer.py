"""iOS IPA binary SAST: symbols, strings and Mach-O metadata of the app and its own frameworks."""
...
class ParsedMachO:
    """Holds a Mach-O slice together with its FatBinary; a slice outliving its parent segfaults."""
...
_UNCONFIRMED_ORIGIN_SUMMARY = (
    "could not confirm whether this framework is first-party or third-party; "
    "check before treating it as the app's own code"
)
...
def _tag_unconfirmed_origin(findings: List[Dict[str, Any]], reason: str) -> None:
    ...
    for f in findings:
        ...
        f["details"]["description"] = (
            f"{base_desc} Caution: {_UNCONFIRMED_ORIGIN_SUMMARY} ({reason})."
        ).strip()
...
def analyze_ipa(ipa_path: str, override_config: Optional[ScopeConfig] = None) -> Dict[str, Any]:
    """Analyze one .ipa. override_config wins over the automatic framework classification."""
    result: Dict[str, Any] = {
        "ok": False,
        "error": None,
        "encrypted": False,
            ...
            with tempfile.TemporaryDirectory(prefix="narvy-ios-") as tmpdir:
                ...
                if encrypted:
                    result["encrypted_message"] = (
                        "This IPA is FairPlay-encrypted (App Store DRM). This local run reads "
                        "its cleartext surface: Info.plist, entitlements, URL schemes / ATS, "
                        "linked and embedded frameworks, Objective-C metadata; `narvy scan "
                        "--upload` adds ML triage and MASVS mapping on that surface. The app's "
                        "own code is encrypted in the file. Narvy dynamic analysis runs the app "
                        "on a real device, where it is decrypted, and covers that code for most "
                        "apps; apps with strong anti-tampering protection can block the "
                        "instrumentation, and the report then says so."
                    )
                ...
                if fw_names:
                    result["notes"].append(
                        f"Embedded frameworks: {len(fw_names)} found, {analyzed_fw} analyzed "
                        f"({unconfirmed_fw} of unconfirmed origin), {skipped_fw} known "
                        f"third-party SDKs skipped.")
