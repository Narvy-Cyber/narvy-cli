"""Dependency CVEs for web/backend repos from manifests and lockfiles, via OSV."""
...
# yarn.lock header: comma-separated "name@range" entries ending in ':'. The version
# is on an indented `version "x"` (v1) or `version: x` (berry) line.
...
# Exact Gemfile pins only (`gem 'rails', '8.1.3.1'`); ranges start with an
...
# Manifest filenames per ecosystem, to tell "no manifest" ('none') from
# "manifest present but unresolved" ('partial').
...
def _coverage_gaps(source_dir: str, deps: List[_WebDep]) -> List[str]:
    """Manifests that yielded no resolvable deps, plus a note for a Gemfile without a lock."""
