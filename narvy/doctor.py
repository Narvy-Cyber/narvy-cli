"""`narvy doctor`: preflight environment check.
Prints a PASS/FAIL list with a concrete fix for each failure."""
import os
import platform
import re
import shutil
...
def _check_semgrep():
    """Informational: semgrep drives the deep AST/taint pass. A broken install leaves it off
    PATH and `scan` silently falls back to regex-only rules."""
    from . import semgrep_engine
    if semgrep_engine.is_available():
        return True, f"{shutil.which('semgrep')}", None
    return True, "Not on PATH - scans fall back to regex-only (reduced coverage)", (
...
def _check_nuclei():
    ...
    return True, f"{nuclei_bin} ({version.splitlines()[0][:60]})", None
...
def run_doctor():
    ...
    for name, hint in optional_hints:
        console.print(f"\n[bold cyan]{name} (optional):[/bold cyan] {_rich_escape(hint)}")
