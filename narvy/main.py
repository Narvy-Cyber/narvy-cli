...
def cmd_login(args):
    ...
    if status != 200:
        console.print(f"[bold red]Login to {uploader.server_host()} failed:[/bold red] {data.get('error', status)}")
...
def cmd_scans(args):
    ...
    if status != 200:
        ...
        console.print(f"[bold red]Could not fetch scan history from {uploader.server_host()} ({status}):[/bold red] {error_msg}")
        raise SystemExit(EXIT_ERROR)
    ...
    table = Table(title=f"Recent scans on {rich_escape(_server_and_account(creds))}")
...
def _server_and_account(creds):
    """'narvy.io (you@example.com, starter)'; NARVY_TOKEN alone carries no email or plan."""
    parts = [p for p in ((creds or {}).get("email"), (creds or {}).get("plan")) if p]
    host = uploader.server_host()
    return f"{host} ({', '.join(parts)})" if parts else host


def _upload_failed_label(status):
    return f"Upload to {uploader.server_host()} failed ({f'HTTP {status}' if status else 'no response'}):"
...
def _submit_results_ingest(all_findings, scan_mode, target_path):
    ...
    console.print(
        f"[cyan]Uploading {len(all_findings)} finding(s) to {rich_escape(_server_and_account(creds))} "
    ...
    try:
        ...
        console.print(f"[bold red]Upload to {uploader.server_host()} failed:[/bold red] {e}")
        raise SystemExit(EXIT_ERROR)
    ...
    if status not in (200, 201):
        ...
        console.print(f"[bold red]{_upload_failed_label(status)}[/bold red] {error_msg}")
...
_UNREADABLE_SKIP_DIRS = frozenset({
    ".git", "node_modules", ".gradle", ".idea", "Pods", "Carthage", "__pycache__",
    ".venv", "venv", "DerivedData", ".build",
})


def _unreadable_paths(source_dir):
    """Files and directories under source_dir this user cannot read (vendored dirs aside)."""
    bad = []

    def _onerror(err):
        if getattr(err, "filename", None):
            bad.append(os.path.relpath(err.filename, source_dir) + os.sep)

    for root, dirs, files in os.walk(source_dir, onerror=_onerror):
        dirs[:] = [d for d in dirs if d not in _UNREADABLE_SKIP_DIRS]
        for name in files:
            full = os.path.join(root, name)
            if not os.path.islink(full) and not os.access(full, os.R_OK):
                bad.append(os.path.relpath(full, source_dir))
    return sorted(bad)
...
def cmd_scan(args):
    ...
    if args.output in ("sarif", "json") and args.file != "-":
        ...
        if _werr:
            ...
            raise SystemExit(EXIT_ERROR)
        own_reports.set_output_path(_preflight_out)
    ...
    unreadable = _unreadable_paths(target_path) if os.path.isdir(target_path) else []
    ...
    # Our own earlier reports (narvy.sarif, results JSON) quote rule text that
    # the secret rules would match; they are not project code.
    if os.path.isdir(target_path):
        all_findings = own_reports.drop_own_reports(all_findings, target_path)
    if own_reports.SKIPPED:
        names = ", ".join(sorted(own_reports.SKIPPED)[:5])
    ...
    if unreadable:
        console.print(
            f"\n[bold yellow]{len(unreadable)} file(s) or folder(s) could not be read (permissions) "
            "and were not scanned.[/bold yellow]"
            + ("" if getattr(args, "verbose", False) else " [dim]Add -v to list them.[/dim]"))
        if getattr(args, "verbose", False):
            for rel in unreadable:
                console.print(f"[yellow]  - {rich_escape(rel)}[/yellow]")
    ...
    if args.upload and not getattr(args, "upload_binary", False):
        ...
        console.print(f"\n[cyan]Uploading {os.path.basename(target_path)} to {rich_escape(_server_and_account(creds))} for the full server-side scan...[/cyan]")
...
def cmd_upload_all(args):
    ...
    console.print(
        ...
        f"Scanning and uploading to {rich_escape(_server_and_account(creds))}...[/cyan]\n"
    )
...
def cmd_web_scan(args):
    ...
    try:
        ...
        console.print(f"[bold red]ERROR:[/bold red] {rich_escape(str(e))}")
        if any(w in str(e) for w in ("loopback", "internal", "private")):
            console.print(
                "[dim]web-scan only scans publicly reachable URLs. Loopback, private-network and "
                "cloud metadata addresses are refused so a scan can never reach internal services. "
                "Point it at the app's public or staging URL.[/dim]")
        raise SystemExit(EXIT_BAD_TARGET)
...
def cmd_cloud_scan(args):
    ...
    if args.format == "json":
        output = json.dumps({"tool": "Narvy Cloud Scanner CLI", **report}, indent=2, default=str)
    elif args.format == "sarif":
...
def cli():
    ...
    p_doctor = sub.add_parser(
        "doctor", allow_abbrev=False, help="Check your Java/jadx/network/iOS-tooling setup before scanning",
        epilog="exit codes:\n  0  every required check passed (missing optional tools are INFO only)\n"
    ...
    p_scan.add_argument("-v", "--verbose", action="store_true",
                        help="List every file the scan could not read, not just the count.")
