"""SSRF defense for the web scanner: URL validation, IP pinning, and
redirect-aware safe fetching.
"""
...
except Exception:  # pragma: no cover - loguru always present in prod
    import logging
...
class SSRFSafeAdapter(HTTPAdapter):
    """HTTPAdapter that pins the TCP connection to a pre-validated IP, keeping the
    hostname for Host header/SNI so it cannot be re-resolved between validate and connect."""
...
def safe_get(
    ...
    **kwargs,
    ...
    """SSRF-safe GET: every redirect hop is re-validated and re-pinned. Cookies and
    headers from ``session`` are preserved across hops."""
...
def safe_session(
    ...
    *,
    ...
    """Return (session, validated_url) pinned to ``url``'s IP, for one request.
    Callers that follow redirects must use safe_get, which re-validates each hop."""
