...
def _is_prefkey_echo_or_typename(line: str) -> bool:
    ...
    if not lhs:
        return False
    name = lhs.group(1)
    # Echo: the value repeats the constant name, or the constant is a KEY_*/PREF_*
    # preference-key holding a settings-store key slug (e.g. KEY_SECRET_TOKEN =
    # "secret_token_pref").
    return val.lower() in name.lower() or bool(_PREFKEY_CONST_RE.match(name))


def _has_no_string_literal_rhs(line: str) -> bool:
    """A name-based secret match whose right-hand side has NO string literal - a
    function call or identifier (e.g. `val apiKey = getApiKeyOrDefault()`), not a
    hardcoded secret. A real hardcoded secret is always a quoted literal."""
