"""ELF hardening checks (NX, PIE, RELRO, canary, RPATH) for native libs in an APK/AAB."""
...
def _canary_evidence_note(facts: _SoFacts) -> str:
    ...
    return (
        "Not verified: this scanner can't decode code for this architecture, "
        "so the missing symbol is not corroborated. Check the build flags "
        "before acting on it. "
    )
