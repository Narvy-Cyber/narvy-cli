...
rm -rf venv
python3 -m venv venv
source venv/bin/activate   # NOTE: bin/, not Scripts/ - only Windows differs here
pip install -e .
narvy --help
```
...
# 3. Full scan tied to your account, visible on the dashboard
...
- Step 3 prints "Server scan submitted. scan_id=..." and that scan appears
  on narvy.io under your account within a few seconds.
...
**Scope of this feature (honest limitation):** wired into the Android
...
- **GitHub Advisory Database** (iOS only, `ecosystem=swift`) as a
  ...
  thin. **Honest constraint, not silently ignored:** GitHub's REST API is
...
### `narvy web-scan` - passive-only Nuclei scan
...
**Safety posture - not just a default, structurally enforced:** this
scanner NEVER sends active/injection payloads (no SQLi/XSS/SSTI fuzzing,
no `-dast` pass). There is no flag to turn that on. It also refuses to scan
Narvy's own domains and a curated list of major third-party platforms
(`narvy/web/scan_blocklist.py`), and it validates + pins the
target's resolved IP before ever connecting (`narvy/web/
ssrf_guard.py`) so it can't be tricked into hitting an internal/private
address via DNS rebinding or a redirect. Active/aggressive scanning needs
verified domain ownership and only runs from the Narvy dashboard.
...
| 2 | the gate could NOT be run honestly, so it refuses to pass the build |
