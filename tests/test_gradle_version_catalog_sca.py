"""SCA coverage for Gradle version catalogs (gradle/libs.versions.toml):
`libs.*` dependency declarations resolve to concrete Maven coordinates so they
reach the OSV CVE lookup instead of silently resolving to zero dependencies.
"""
