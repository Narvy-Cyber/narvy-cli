"""Regression tests for the precision/recall batch:

1. comment-stripping post-filter (iOS + Android source regex rules)
2. duplicate JWT-secret rule de-dupe + test/fixture secret down-weighting (web)
3. iOS trust-all-certs downgrade behind an opt-in flag
4. iOS Swift hardcoded cloud-credential rules (recall gap)

These test the pure helpers directly (no semgrep / no network) plus one
end-to-end regex pass through the shared rule engine.
"""
import os
import tempfile
