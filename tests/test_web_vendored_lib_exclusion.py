"""Bundled third-party JS libraries vendored under static/ or assets/ (jquery,
vue, tinymce, d3, leaflet, select2, pdf.js, *.min.js) must not flood CRITICAL
findings, while the scanned app's OWN code keeps its severity.
"""
...
def test_app_code_named_like_a_lib_outside_served_root_is_kept():
    # Regression for over-exclusion: a basename glob (vue.js/jquery.js/d3.js) or a
    # lib dir-name (leaflet/) must NOT drop app code that lives outside static/,
    # assets/, public/, ... . These are the scanned app's own files.
    with tempfile.TemporaryDirectory() as root:
...
def test_vendored_flood_under_served_root_stays_suppressed():
    # The other direction: same lib names UNDER a served-asset root are vendored
    # third-party code and must remain suppressed even though they carry sinks.
    with tempfile.TemporaryDirectory() as root:
        _write(root, "package.json", '{"name":"x","version":"1.0.0"}')
