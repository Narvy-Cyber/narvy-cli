"""nuclei timeout scales with --rate-limit, capped, with a floor."""
import pytest
...
def test_budget_covers_the_measured_overhead_ratio():
    """The budget covers the measured overhead at every rate limit."""
...
def test_active_budget_fits_a_full_active_run():
    # A full active run against a local static server took about 725s at 50 req/s.
    from narvy.web.scanner import ACTIVE_REQUEST_ESTIMATE
    assert compute_subprocess_timeout(ACTIVE_REQUEST_ESTIMATE, 50) >= 900
