"""android-weak-prng grading: HIGH, INFO or LOW by context, never dropped."""
import os
import sys
import tempfile
...
def _classify_all(fixtures, rel_dir):
    """Grade each fixture's weak-PRNG lines directly, no semgrep needed."""
    out = {}
...
def test_security_context_stays_high():
    """Credentials, tokens, IVs and OTPs from a weak PRNG stay HIGH."""
...
def test_non_security_context_downgraded_to_info():
    """Jitter, sampling, backoff and colours are not CWE-338."""
    graded = _classify_all(
        NON_SECURITY_FIXTURES,
...
def test_sink_evidence_beats_security_package_path():
    """A sleep jitter in a crypto package isn't HIGH because of the path."""
...
def test_duplicate_same_file_line_collapsed():
    """Many matches on one line collapse to one finding."""
    dupes = [
...
def test_unreadable_file_keeps_original_severity():
    """Unreadable file: keep the original severity."""
    findings = [
