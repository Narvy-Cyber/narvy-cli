"""Tests for split-APK bundle support (.apkm / .xapk / .apks): detection,
member classification, extraction, safety limits and pipeline wiring.
"""
...
def _apk_bytes(*, dex=0, so=(), arsc=False, manifest=True, extra=()):
    """A minimal APK: a ZIP whose member names are what every check in
    split_bundle/_validate_base_apk keys on."""
    buf = io.BytesIO()
...
def make_apkm(path):
    """base.apk with all the dex and no .so, split_config.* for ABI/density,
    info.json metadata."""
    return _bundle(
        path,
        [
...
def make_xapk(path):
    """Base named after the package, config.* splits, dex-bearing dynamic
    feature modules, and a feature-owned ABI split."""
    return _bundle(
        path,
        [
...
def test_detect_scan_mode_still_rejects_other_archives(tmp_path):
    """Adding bundle formats must not turn every zip-shaped file into an
    Android target."""
...
def test_abi_preference_is_deterministic_not_zip_order(tmp_path):
    """arm64-v8a wins over armeabi-v7a regardless of member order, matching
    native_hardening._pick_arch."""
    bundle_path = _bundle(
        str(tmp_path / "b.apkm"),
        [
...
def test_base_module_that_ships_its_own_so_needs_no_abi_split(tmp_path):
    """When .so live inside the app module, pulling in an ABI split too would
    double-report the same libraries."""
    bundle_path = _bundle(
...
def test_xapk_base_named_after_package(tmp_path):
    """This shape writes <package_name>.apk instead of base.apk, read from the
...
def test_feature_owned_abi_split_is_not_mistaken_for_the_apps(tmp_path):
    """Picking the feature-owned split would analyse a feature module's
    libraries and report nothing about the app's own."""
    bundle_path = make_xapk(str(tmp_path / "app.xapk"))
    eb = split_bundle.extract_for_scan(bundle_path, str(tmp_path / "out"))
...
def test_extracted_feature_names_cannot_collide_with_base(tmp_path):
    """A feature module literally named base.apk (or matching the ABI split's
    basename) must not overwrite what was already written to dest_dir."""
    bundle_path = _bundle(
...
def test_apks_master_choice_is_stable_across_member_order(tmp_path):
    """base-master.apk vs base-master_2.apk are the same code twice; which
    one is scanned must not depend on zip order (it would make finding line
...
def test_config_split_is_never_chosen_as_the_base(tmp_path):
    """No base.apk, no usable metadata, and the largest member is a config
    split: the app module must still win, because only it has a classes.dex."""
    bundle_path = _bundle(
...
def test_bundle_with_no_app_module_refuses_instead_of_scanning_a_split(tmp_path):
    """Nothing here has a dex, so scanning a config split would report a
...
def test_oversized_feature_module_is_reported_as_incomplete(tmp_path, monkeypatch):
    """A feature module that cannot be added to the run must be reported as
...
def test_bundle_scan_delegates_to_the_same_local_scan(tmp_path, monkeypatch):
    """_run_split_bundle_scan must call the same _run_local_scan every .apk
    goes through, with the app module, ABI split and feature modules."""
...
def test_decompile_apk_passes_extra_inputs_to_jadx(monkeypatch, tmp_path):
    """jadx merges multiple input files into one sources tree; that is how
    feature-module code gets scanned."""
