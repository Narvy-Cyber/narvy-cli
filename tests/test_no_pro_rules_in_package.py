"""Private rules and corpus never reach git, the package config or a built wheel."""
import glob
import os
import subprocess
import sys
...
def test_no_private_material_is_tracked_by_git():
    """No private path is tracked by git."""
    offenders = []
...
def test_loader_works_without_the_pro_tree(tmp_path, monkeypatch):
    """Without the private tree the loader returns base packs only."""
...
def test_built_wheel_contains_no_private_material():
    """Build a real wheel (isolated, needs setuptools>=61) and check its contents."""
