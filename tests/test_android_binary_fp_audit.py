"""Android binary false-positive tests: stack-protector evidence, JNI-namespace
vendor classification, and the PendingIntent / exported-component / Google API
key rule gates.
"""
import os
import shutil
...
def test_controlled_compile_proves_the_ambiguity():
    """-fstack-protector-strong emits `__stack_chk_fail` only when a function
    actually has a stack buffer, so symbol presence alone cannot support the
    finding."""
...
def test_aarch64_pure_register_spill_frame_is_not_instrumentable():
    """A function whose buffer is on the heap (`bl malloc`), with the stack
    used only to spill x29/x30/x19. Nothing for the protector to protect."""
    code = _a64([
...
def test_aarch64_escaping_local_address_is_instrumentable():
    """`add x1, sp, #0x18` passes the address of a stack local, which is what
    -fstack-protector-strong instruments, so a missing canary there is a
    genuine finding and must survive the gate."""
    code = _a64([
...
def test_aarch64_frame_pointer_setup_alone_is_not_evidence():
    """`add x29, sp, #imm` and `sub sp, sp, #imm` are frame bookkeeping, not an
    escaping address; counting them would make the gate a no-op."""
...
def test_unknown_architecture_returns_none_not_false():
    """"No decoder" must never be reported as "nothing instrumentable", which
    would silently suppress findings on x86/x86_64."""
    class _Hdr:
        machine_type = object()
...
def test_androidx_jni_namespace_is_vendor_whatever_the_filename():
    """libdatastore_shared_counter.so's name gives no hint that it is AndroidX;
    its JNI entry points say so unambiguously."""
...
def test_mixed_namespaces_are_not_vendor():
    """A library that also exports the app's own JNI entry points is the app's
    own build, even if it embeds a vendor shim."""
...
def test_pending_intent_with_immutable_flag_is_not_reported(tmp_path, flags, label):
    """The numeric values are the literal flag arguments jadx emits at real
    call sites."""
    src = (
        "class A {\n"
...
def test_deeplink_activity_is_still_reported(tmp_path):
    """A BROWSABLE deep-link handler is attacker-reachable; only LAUNCHER and
    LEANBACK_LAUNCHER entry points are exempt."""
...
def test_firebase_generated_key_is_medium_not_high(tmp_path):
    """Google documents this key as an identifier, not a secret, and the
    residual risk of an unrestricted key is not visible from the binary."""
...
def test_google_key_hardcoded_in_source_stays_high(tmp_path):
    """A raw AIza key in code is a developer mistake, not the documented
