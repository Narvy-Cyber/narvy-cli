"""Tests that "has a Gradle file" is not "is an Android project": every real
Android project shape still routes to android-source, non-Android JVM projects
route to web-source, and a polyglot JVM+JS repo resolves both stacks.
"""
...
def _make_classic_android_app(root: str) -> None:
    """Root Gradle files declaring AGP plus an app module with its own
    manifest."""
...
def _make_agp_library_module_without_manifest(root: str) -> None:
    """Since AGP 7 a library module can ship no AndroidManifest.xml at all
    (`namespace` moved into the Gradle DSL), so the plugin id is the only
    Android evidence in the tree."""
...
def _make_version_catalog_android_app(root: str) -> None:
    """With a Gradle version catalog the literal `com.android.application`
    string exists only in gradle/libs.versions.toml."""
    _write(os.path.join(root, "build.gradle.kts"),
...
def _make_convention_plugin_android_app(root: str) -> None:
    """AGP wired through the repo's own convention plugins, so the literal
    `com.android.*` id appears in no module build file and the `android { }`
    DSL block is the only surviving marker."""
...
def _make_androidx_properties_only_app(root: str) -> None:
    """gradle.properties carrying the AndroidX flags, accepted as a marker for
    repos whose manifest sits deeper than the walk bound."""
...
def _make_plain_java_library(root: str) -> None:
    """A `java-library` Gradle project: plain Java, used by an Android app but
    not itself an Android project."""
    _write(os.path.join(root, "build.gradle.kts"), 'plugins {\n    `java-library`\n}\n')
...
def _make_polyglot_jvm_plus_js(root: str) -> None:
    """Gradle Spring Boot backend plus a ui/ front-end with its own
    package.json. Must land on web-source."""
...
def test_android_app_with_package_json_still_beats_web_source():
    """The tightened Android check must not hand a React-Native-ish Android
    repo to web-source."""
    with tempfile.TemporaryDirectory() as d:
        _make_classic_android_app(d)
...
def test_jvm_dependency_coordinates_do_not_fake_an_android_marker():
    """A JVM backend that merely mentions a com.android coordinate or the word
    android in a comment is still not an Android project."""
    with tempfile.TemporaryDirectory() as d:
        _make_gradle_spring_boot(d)
...
def test_jvm_non_android_resolves_to_the_java_stack_not_an_empty_scan():
    """Routing a JVM backend to web-source only helps if the analyzer then
    resolves the Java packs; otherwise the scan runs zero configs and reports
    a clean tree."""
    for maker in (_make_gradle_spring_boot, _make_maven_spring_boot,
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            assert "java_narvy" in names, (maker.__name__, names)
...
def test_polyglot_jvm_plus_js_resolves_both_stacks():
    """Both halves must resolve, so neither language is silently the only one
    scanned."""
    with tempfile.TemporaryDirectory() as d:
        _make_polyglot_jvm_plus_js(d)
        ...
        assert "java_narvy" in names, names


def test_supported_languages_label_claims_java_only_with_routing_in_place():
    """The label and the routing must agree, in whichever direction they are
    set."""
...
def test_csharp_is_a_real_ported_stack_not_the_remaining_gap():
    """A .NET repo gets real AST coverage, not just the always-on packs'
