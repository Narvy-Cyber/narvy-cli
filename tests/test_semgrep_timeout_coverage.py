"""Semgrep run budget scales with tree size, stays bounded, and a timeout or
error is recorded as degraded coverage instead of a clean zero.
"""
...
class DegradedCoverageTests(unittest.TestCase):
    ...
    def test_timeout_is_not_reported_as_clean(self):
        ...
        with mock.patch.object(semgrep_engine, "is_available",
                               return_value=True), \
             mock.patch("narvy.semgrep_engine.subprocess.run",
...
class SwiftPathTests(unittest.TestCase):
    ...
    def test_swift_timeout_records_degraded(self):
        ...
        with mock.patch.object(semgrep_engine, "is_available",
                               return_value=True), \
             mock.patch(
                 "narvy.ios.source_analyzer.subprocess.run",
