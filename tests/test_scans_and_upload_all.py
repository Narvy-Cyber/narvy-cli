"""Tests for `narvy scans` (request shape and error rendering) and
`narvy upload-all` (target discovery, login pre-check, and batch
resilience when one target fails).
"""
...
def test_list_scans_builds_expected_request(monkeypatch):
    """GET /api/v1/scans with X-API-Token, limit/offset and an optional status
    filter."""
    captured = {}
...
def test_list_scans_non_json_body_does_not_crash(monkeypatch):
    """A 404 handler returns an HTML body, so r.json() raises ValueError and
    list_scans() must catch it."""
    def fake_get(url, headers=None, params=None, timeout=None):
...
def test_cmd_upload_all_one_failure_does_not_abort_the_batch(tmp_path, monkeypatch):
    """A batch tool that dies on the first bad app defeats the point of
    batching."""
    ...
    assert len(calls) == 2
    # overall exit code is 1 because at least one target failed
    assert exc.value.code == 1
    out = buf.getvalue()
