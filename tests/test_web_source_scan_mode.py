"""Web-source mode: detection vs Android/iOS, real findings, --upload rejection."""
...
def test_classic_php_project_without_composer_json_is_web_source():
    """Plain .php files without composer.json are web-source."""
    with tempfile.TemporaryDirectory() as d:
...
def test_dotnet_maui_project_with_android_manifest_is_web_source():
    """A MAUI app with a generated Android manifest is web-source."""
    with tempfile.TemporaryDirectory() as d:
...
def test_real_android_gradle_project_still_wins_over_dotnet_marker():
    """A real Android Gradle build beats a stray .csproj."""
    with tempfile.TemporaryDirectory() as d:
...
def test_deeply_nested_manifest_is_detected_as_web_source():
    """A manifest 4 levels deep is still detected."""
    with tempfile.TemporaryDirectory() as d:
...
def test_nested_xcodeproj_does_not_flip_web_repo_to_ios():
    """A nested example .xcodeproj doesn't flip a web repo to iOS."""
    with tempfile.TemporaryDirectory() as d:
...
def test_sca_zero_deps_is_not_reported_complete():
    """Zero resolved deps is never 'complete'."""
