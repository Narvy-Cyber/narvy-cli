"""The `--fail-on` build gate: unrankable severities, coverage gaps and
unreachable web targets must never be reported as a pass.

Exit codes: 0 = full coverage and nothing at or above the threshold,
1 = the gate tripped, 2 = the gate could not be run honestly.
"""
...
def test_rate_limit_floor_cannot_be_escaped_with_a_minus_sign():
    """nuclei treats a non-positive -rate-limit as no rate limiting at all, so
    min() alone would leave WEB_MAX_RATE_LIMIT bypassable with a negative number."""
