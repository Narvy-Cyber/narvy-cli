"""Hybrid mobile repos: RN/Capacitor go web-source with a note, Flutter stays android-source."""
...
def test_expo_monorepo_with_scattered_native_modules_is_web_source():
    # Native code only under modules/x/android and modules/x/ios still counts as
    # a JS-primary hybrid, with a tree-wide note.
    with tempfile.TemporaryDirectory() as root:
...
def test_native_android_with_stray_root_package_json_stays_android():
    # A root Gradle project with a tooling package.json and a helper .js must
    # stay android-source.
    with tempfile.TemporaryDirectory() as root:
